...
const addValues = (arr) => {
  let sum = arr.reduce((acc,num,idx)=>{
    acc = acc + num;
    return acc;
  },0);  
  return sum;
...
const addPurchases = (arr) => {
  let sum = arr.reduce((acc,item,idx)=>{
...
const countNumberOfElements = (arr) => {
  let count = arr.reduce((acc,item,idx)=>{
    acc = acc + 1;
    return acc;
  },0);
  return count;
...
const returnNames = (arr) => {
  let names = arr.reduce((acc,item,idx)=>{
    acc.push(item.name);
    return acc;},[]);
    return names;
};
...
const reversedString = (str) => {
  let newStr = str.split('').reduce((acc,letter,idx)=>{
    acc = letter + acc;
    return acc;
  },"");
  return newStr;
